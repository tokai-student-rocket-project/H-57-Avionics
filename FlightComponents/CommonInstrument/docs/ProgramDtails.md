...
まず、測定したい電圧は3.3V,5V,12Vの3種類です。これらを最大電圧とします。Arduino MKR WAN 1310のADC(Analog Digital Converter)は3.3Vの参照電圧までしか入力できないため、3つの電圧を参照電圧以下に下げる必要があります。ここでは、この電圧を2.4Vの基準電圧とします。
...
$$ 測定電圧[\text{V}] = \text{ADC}測定値 \times \frac{1}{2^{分解能[\text{bit}]}} \times 参照電圧[\text{V}] \times 電圧係数 $$
