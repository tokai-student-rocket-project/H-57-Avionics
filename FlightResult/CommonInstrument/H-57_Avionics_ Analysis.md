...
当初は方式1を運用していたが、方式1は内部電源操作を機体組み後も行う必要があり、内部電源配線が機体後方のバルブカプラまで伸びていた。これにより、飛行中の振動や衝撃により瞬電を起こす懸念があり、2/28リハは方式2で運用した。

しかし、方式2はシークエンス中の内部電源消費が大きく、供給電圧が低下したことで撤収時にはボールバルブのサーボが低電圧特有の挙動（勝手に開閉を繰り返す）を示したため、以降は方式1に戻して運用を行った。

方式1で打ち上げを行ったが、結果的には懸念していた飛行中の瞬電は起こらなかった。
